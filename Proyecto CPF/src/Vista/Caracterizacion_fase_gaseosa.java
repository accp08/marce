...
public class Caracterizacion_fase_gaseosa {
	...
	public static void main(String[] args) {
		...
		Double Temperatura =(double)560;
		Double Presion =(double)1000;
		...
		double [] Temperatura_critica_componetes_gas=new double [numero_de_compuestos];
		double [] Presion_critica_componetes_gas=new double [numero_de_compuestos];
		...
		for (int i=0; i<lista_de_compuestos.size();i++){
			Temperatura_critica_componetes_gas[i]= lista_de_compuestos.get(i).getTemperatura_critica_Tc()+460;
			Presion_critica_componetes_gas[i]= lista_de_compuestos.get(i).getPresión_critica_Pc();
		...
		for (int i=lista_de_compuestos.size(); i<numero_de_compuestos;i++){
			Temperatura_critica_componetes_gas[i]= T_criticas_no_caracterizables[i-lista_de_compuestos.size()];
			Presion_critica_componetes_gas[i]= P_criticas_no_caracterizables[i-lista_de_compuestos.size()];
		...
		for (int i = 0; i <numero_de_compuestos; i++) {
			if (Fraccion_molar_fase_gaseosa[i]!=0) {
				System.out.println(i+ " "+ Temperatura_critica_componetes_gas[i]);
			}
		}
		...
		Double Temperatura_pseudoreducida_Tr=(double)0;
		...
		for (int i=0;i<numero_de_compuestos;i++){
			Temperatura_pseudocritica_Tc=Temperatura_pseudocritica_Tc+Temperatura_critica_componetes_gas[i]*Fraccion_molar_fase_gaseosa[i];
			Presion_pseudocritica_Pc=Presion_pseudocritica_Pc+Presion_critica_componetes_gas[i]*Fraccion_molar_fase_gaseosa[i];
		...
		for(int i=0;i<numero_de_compuestos;i++){
			sumatoria_1=sumatoria_1+Fraccion_molar_fase_gaseosa[i]*Temperatura_critica_componetes_gas[i]/Presion_critica_componetes_gas[i];
			sumatoria_2=sumatoria_2+Fraccion_molar_fase_gaseosa[i]*Math.pow(Temperatura_critica_componetes_gas[i]/Presion_critica_componetes_gas[i],0.5);
		}
		...
		for(int i=lista_de_compuestos.size();i<numero_de_compuestos;i++){
			...
			parametro_ek [i]=(Temperatura_critica_componetes_gas[i]/Math.pow(Presion_critica_componetes_gas[i],0.5))*(0.3129*Fraccion_molar_fase_gaseosa[i]-4.8156*Math.pow(Fraccion_molar_fase_gaseosa[i], 2)+27.3751*Math.pow(Fraccion_molar_fase_gaseosa [i], 3));
		}
		for(int i=lista_de_compuestos.size();i<numero_de_compuestos;i++){
		...
		Temperatura_pseudocritica_Tc=Temperatura_pseudocritica_Tc+Math.pow(parametro_K,2)/parametro_J;
		Presion_pseudocritica_Pc=Presion_pseudocritica_Pc+(Math.pow(parametro_K,2)/parametro_J)/parametro_J;
		...
		Temperatura_pseudocritica_Tc=Temperatura_pseudocritica_Tc-80*fraccion_molar_Co2+130*fraccion_molar_H2S-250*fraccion_molar_N2;
		Presion_pseudocritica_Pc=Presion_pseudocritica_Pc+440*fraccion_molar_Co2+600*fraccion_molar_H2S-170*fraccion_molar_N2;

		Temperatura_pseudoreducida_Tr=Temperatura/Temperatura_pseudocritica_Tc;
		Presion_pseudoreducida_Pr=Presion/Presion_pseudocritica_Pc;


		//System.out.println("Tpc = "+ Temperatura_pseudocritica_Tc+"  ºR");
		//System.out.println("Ppc = "+ Presion_pseudocritica_Pc+"  psia");
//		System.out.println("Tpr = "+ Temperatura_pseudoreducida_Tr);
//		System.out.println("Ppr = "+ Presion_pseudoreducida_Pr);
		...
		Double Ecuacion_1=((-0.27*Presion_pseudoreducida_Pr/(X*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X,5)+C_A10*(1+C_A11*Math.pow(X,2))*Math.pow(X,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X,2))+1);
		Double D_1=((-0.27*Presion_pseudoreducida_Pr/(X_1*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X_1+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_1,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_1,5)+C_A10*(1+C_A11*Math.pow(X_1,2))*Math.pow(X_1,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X_1,2))+1);
		Double D_2=((-0.27*Presion_pseudoreducida_Pr/(X_2*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X_2+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_2,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_2,5)+C_A10*(1+C_A11*Math.pow(X_2,2))*Math.pow(X_2,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X_2,2))+1);
		...
		while (Math.abs(Ecuacion_1)>Tolerancia || X<0 ){
			X_1=X+delta;
			X_2=X-delta;
			Ecuacion_1=((-0.27*Presion_pseudoreducida_Pr/(X*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X,5)+C_A10*(1+C_A11*Math.pow(X,2))*Math.pow(X,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X,2))+1);
			D_1=((-0.27*Presion_pseudoreducida_Pr/(X_1*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X_1+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_1,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_1,5)+C_A10*(1+C_A11*Math.pow(X_1,2))*Math.pow(X_1,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X_1,2))+1);
			D_2=((-0.27*Presion_pseudoreducida_Pr/(X_2*Temperatura_pseudoreducida_Tr))+(C_A1+C_A2/Temperatura_pseudoreducida_Tr+C_A3/Math.pow(Temperatura_pseudoreducida_Tr, 3)+C_A4/Math.pow(Temperatura_pseudoreducida_Tr, 4)+C_A5/Math.pow(Temperatura_pseudoreducida_Tr, 5))*X_2+(C_A6+C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_2,2)-C_A9*(C_A7/Temperatura_pseudoreducida_Tr+C_A8/Math.pow(Temperatura_pseudoreducida_Tr,2))*Math.pow(X_2,5)+C_A10*(1+C_A11*Math.pow(X_2,2))*Math.pow(X_2,2)/Math.pow(Temperatura_pseudoreducida_Tr,3)*Math.exp(-C_A11*Math.pow(X_2,2))+1);
		...
		Factor_de_compresibilidad_gas=0.27*Presion_pseudoreducida_Pr/(Temperatura_pseudoreducida_Tr*Densidad_reducida_del_gas);
		...
		Densidad_gas=Presion*Peso_molecular_MW_gas/(Factor_de_compresibilidad_gas*10.73*Temperatura);
		Densidad_reducida_del_gas=0.27*(Presion_pseudoreducida_Pr)/(Factor_de_compresibilidad_gas*Temperatura_pseudoreducida_Tr);
		...
		Flujo_de_gas_estandar=35.37*Presion/(Factor_de_compresibilidad_gas*Temperatura)*Flujo_de_gas;
		...
		Double Parametro_em=(5.4402*Math.pow(Temperatura_pseudocritica_Tc,0.16666667)/(Math.pow(Peso_molecular_MW_gas,0.5)*Math.pow(Presion_pseudocritica_Pc, 0.666667)));

		if (Temperatura_pseudoreducida_Tr<=1.5){
			Viscosidad_del_gas_condiciones_estandar=(34e-5*Math.pow(Temperatura_pseudoreducida_Tr,0.8888889)/Parametro_em);
		}
		else{
			Viscosidad_del_gas_condiciones_estandar=(166.8e-5*Math.pow((0.1338*Temperatura_pseudoreducida_Tr-0.0932), 0.5555556)/Parametro_em);
		}
