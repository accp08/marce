...
public class Caracterizacion_crudo {
	...
	public static void main(String[] args) {
		...
		Double Temperatura_crudo =(double)131+460;
		Double Presion_crudo =(double)1426;
		Double Flujo_de_crudo=(double)500;
		int numero_de_compuestos =lista_de_compuestos.size()+numero_de_compuestos_no_caracterizables;
		double [] Fraccion_molar_crudo_sin_agua= new double [numero_de_compuestos];
		...
		double [] Temperatura_crudo_critica_componetes_liquido=new double [numero_de_compuestos];
		double [] Presion_crudo_critica_componetes_liquido=new double [numero_de_compuestos];
		...
		for (int i=0; i<lista_de_compuestos.size();i++){
			Temperatura_crudo_critica_componetes_liquido[i]= lista_de_compuestos.get(i).getTemperatura_critica_Tc()+460;
			Presion_crudo_critica_componetes_liquido[i]= lista_de_compuestos.get(i).getPresión_critica_Pc();
		...
		for (int i=lista_de_compuestos.size(); i<numero_de_compuestos;i++){
			Temperatura_crudo_critica_componetes_liquido[i]= T_criticas_no_caracterizables[i-lista_de_compuestos.size()];
			Presion_crudo_critica_componetes_liquido[i]= P_criticas_no_caracterizables[i-lista_de_compuestos.size()];
		...
		Double Presion_crudo_de_saturacion=(double)0;
		Double Tension_superficial=(double)0;
		Double Temperatura_crudo_pseudocritica_Tc=(double)0;
		Double Presion_crudo_pseudocritica_Pc=(double)0;
		...
		Double Temperatura_crudo_pseudoreducida_Tr_liquido=(double)0;
		Double Presion_crudo_pseudoreducida_Pr=(double)0;
		...
		for(int i=0;i<numero_de_compuestos;i++){
			Fraccion_masica_liquido[i]=Fraccion_molar_crudo_sin_agua[i]*Peso_molecular_liquido[i]/Peso_molecular_MW_crudo;
		...
		for (int i=0;i<numero_de_compuestos;i++){
			Temperatura_crudo_pseudocritica_Tc=Temperatura_crudo_pseudocritica_Tc+Temperatura_crudo_critica_componetes_liquido[i]*Fraccion_molar_crudo_sin_agua[i];
			Presion_crudo_pseudocritica_Pc=Presion_crudo_pseudocritica_Pc+Presion_crudo_critica_componetes_liquido[i]*Fraccion_molar_crudo_sin_agua[i];
		}
		Temperatura_crudo_pseudoreducida_Tr_liquido=Temperatura_crudo/Temperatura_crudo_pseudocritica_Tc;
		Presion_crudo_pseudoreducida_Pr=Presion_crudo/Presion_crudo_pseudocritica_Pc;
		//		System.out.println("Temperatura_crudo pseudoreducida= "+Temperatura_crudo_pseudoreducida_Tr_liquido+"  ºR");
		//		System.out.println("Presión pseudocrítica= "+Presion_crudo_pseudoreducida_Pr+"  psia");
		...
		for (int i=0; i<lista_de_compuestos.size();i++){
			parametros_a [i]=Parametro_K [i]*Math.exp(Parametro_N[i]/Temperatura_crudo);
			parametros_b [i]=Parametro_M [i]/10000*Temperatura_crudo+Parametro_C [i];
		}
		//Para los compuestos no caracterizables
		for (int i=lista_de_compuestos.size(); i<numero_de_compuestos;i++){
			parametros_a [i]=Math.exp(3.8405985e-3*Peso_molecular_MW_no_caracterizables[i-lista_de_compuestos.size()]-9.5638281e-4*Peso_molecular_MW_no_caracterizables[i-lista_de_compuestos.size()]/gravedad_especifica_no_caracterizables[i-lista_de_compuestos.size()]+(261.80818/Temperatura_crudo)+7.3104464e-6*Math.pow(Peso_molecular_MW_no_caracterizables [i-lista_de_compuestos.size()],2)+10.753517);
			parametros_b [i]=0.03499274*Peso_molecular_MW_no_caracterizables[i-lista_de_compuestos.size()]-7.2725403*gravedad_especifica_no_caracterizables[i-lista_de_compuestos.size()]+2.232395e-4*Temperatura_crudo-0.016322572*(Peso_molecular_MW_no_caracterizables[i-lista_de_compuestos.size()]/gravedad_especifica_no_caracterizables[i-lista_de_compuestos.size()])+6.2256545;
		}

		for(int i=0;i<numero_de_compuestos;i++){
			Parametro_a=Parametro_a+ parametros_a[i]*Fraccion_molar_crudo_sin_agua[i];
			Parametro_b=Parametro_b+ parametros_b[i]*Fraccion_molar_crudo_sin_agua[i];
		}
		...
		Double Ecuacion_11= Math.pow(X,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X,2)+Parametro_a*X/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
		Double D_11=Math.pow(X_1,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X_1,2)+Parametro_a*X_1/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
		Double D_21=Math.pow(X,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X,2)+Parametro_a*X/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
		...
		while (Math.abs(Ecuacion_11)>Tolerancia1 || X<0 ){
			X_1=X+delta1;
			X_2=X-delta1;
			Ecuacion_11= Math.pow(X,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X,2)+Parametro_a*X/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
			D_11=Math.pow(X_1,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X_1,2)+Parametro_a*X_1/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
			D_21=Math.pow(X_2,3)-(Constante_gas_ideal_R*Temperatura_crudo/Presion_crudo+Parametro_b)*Math.pow(X_2,2)+Parametro_a*X_2/Presion_crudo-Parametro_a*Parametro_b/Presion_crudo;
		...
		Double correcion_densidad_Presion_crudo=(double)((0.167+(16.181)*Math.pow(10, -0.04245*Densidad_crudo))*(Presion_crudo/1000)-0.01*(0.299+263*Math.pow(10,-0.0603*Densidad_crudo))*(Math.pow((Presion_crudo/1000),2)));
		Double correcion_densidad_Temperatura_crudo=(double)((0.0133+152.4*Math.pow((Densidad_crudo-correcion_densidad_Presion_crudo),-2.45))*(Temperatura_crudo-520)-(8.1e-6-0.0622*Math.pow(10,-0.764*(Densidad_crudo+correcion_densidad_Presion_crudo)))*Math.pow(Temperatura_crudo-520,2));
		Densidad_estandar_crudo= Densidad_crudo-correcion_densidad_Presion_crudo+correcion_densidad_Temperatura_crudo;
		...
		Double base=(constante_a*Math.pow(Gravedad_especifica, constante_b)*Math.pow(Gravedad_especifica,constate_c)*Math.pow(Temperatura_crudo,constate_d)*Presion_crudo);
		...
		Presion_crudo_de_saturacion=Math.pow((Solubilidad_del_gas_en_el_liquido/Gravedad_especifica),Parametro_A_BBP_Gaso)*Math.pow(Temperatura_crudo-460,Parametro_B_BBP_Gaso)*Math.pow(Grados_API,Parametro_C_BBP_Gaso);
		Presion_crudo_de_saturacion=Math.pow(10, 1.7669+1.7447*Math.log10(Presion_crudo_de_saturacion)-0.30218*Math.pow(Math.log10(Presion_crudo_de_saturacion),2 ));
		//Correccicón en función de no hidrocarburos.
		Double Correccion_presencia_nitrogeno=1+((-2.65e-4*Grados_API+5.5e-3)*(Temperatura_crudo-460)+0.039*Grados_API-0.8295)*Fraccion_molar_crudo_sin_agua[22]+(1.954e-11*Math.pow(Grados_API,-4.699)*(Temperatura_crudo-460)-4.699*Math.pow(Grados_API, 0.027)-2.366)*Math.pow(Fraccion_molar_crudo_sin_agua[22],2);
		Double Correcion_presencia_dioxido=1-693.8*Fraccion_molar_crudo_sin_agua[24]*Math.pow(Temperatura_crudo-460,-1.553);
		Double Correcion_presencia_sulfuro=1-(0.9035+0.0015*Grados_API)*Fraccion_molar_crudo_sin_agua[23]+0.019*(45-Grados_API)*Math.pow(Fraccion_molar_crudo_sin_agua[23],2);
		Presion_crudo_de_saturacion=Presion_crudo_de_saturacion*Correccion_presencia_nitrogeno*Correcion_presencia_dioxido*Correcion_presencia_sulfuro;
		// System.out.println("Presión de saturación"+Presion_crudo_de_saturacion);
		if (Presion_crudo_de_saturacion<14.7) {
			Presion_crudo_de_saturacion=Presion_crudo;
		}
		...
		Double PX=(double)(Y*Math.pow(Temperatura_crudo-460,-1.163));
		...
		Viscosidad_Beal=Dead_oil_viscosity_Beggs+0.001*(Presion_crudo-Presion_crudo_de_saturacion)*(0.024*Math.pow(Viscosidad_Chew_Connally,1.6)+0.038*Math.pow(Viscosidad_Chew_Connally,0.56));

		//11.1.3) Viscosidad de crudos no saturados.
		Double Viscosidad_Vasquez_Beggs=(double)0;
		Double parametro_a_Vasquez_Beggs=(double)-3.9e-5*Presion_crudo-5;
		Double parametro_m_Vasquez_Beggs=2.6*Math.pow(Presion_crudo,1.187)*Math.pow(10,parametro_a_Vasquez_Beggs);
		Viscosidad_Vasquez_Beggs=Viscosidad_Chew_Connally*Math.pow(Presion_crudo/(Presion_crudo_de_saturacion),parametro_m_Vasquez_Beggs);

		// 11.1.3.1) Determinación de la viscosidad de los componentes individuales.

		double [] Temperatura_crudo_reducida_por_componente= new double [numero_de_compuestos];
		...
		for(int i=0;i<numero_de_compuestos;i++){
			Temperatura_crudo_reducida_por_componente [i]=Temperatura_crudo/Temperatura_crudo_critica_componetes_liquido[i];
			vector_parametros_ei[i]=(5.4402*Math.pow(Temperatura_crudo_critica_componetes_liquido [i], 0.16666666666667))/(Math.pow(Peso_molecular_liquido[i],0.5)*Math.pow(Presion_crudo_critica_componetes_liquido [i], 0.666667));
		}
		for(int i=0;i<numero_de_compuestos;i++){
			if (Temperatura_crudo_reducida_por_componente[i]<1.5){ 
				Viscosidad_por_componente [i]=34e-5*Math.pow(Temperatura_crudo_reducida_por_componente[i],0.94)/vector_parametros_ei[i];
			}
			else{
				Viscosidad_por_componente [i]=(17.78e-5*Math.pow((4.58*Temperatura_crudo_reducida_por_componente[i]-1.67), 0.625))/vector_parametros_ei[i];
			}
		}
		...
		for(int i=0;i<numero_de_compuestos;i++){
			numerador_fraccion=numerador_fraccion+Fraccion_molar_crudo_sin_agua[i]*Viscosidad_por_componente[i]*Math.pow(Peso_molecular_liquido[i],0.5);
			denominador_fraccion=denominador_fraccion+Fraccion_molar_crudo_sin_agua[i]*Math.pow(Peso_molecular_liquido[i],0.5);
		...
		for (int i =lista_de_compuestos.size();i<numero_de_compuestos;i++){
			sumatoria=sumatoria+Fraccion_molar_crudo_sin_agua[i]*Volumen_critico_liquido[i];
		}
		for (int i =0;i<lista_de_compuestos.size();i++){
			sumatoria=sumatoria+Fraccion_molar_crudo_sin_agua[i]*lista_de_compuestos.get(i).getVolumen_critico_Vc()*lista_de_compuestos.get(i).getMasa_molecular();
		}
		...
		vector_parametro_em=5.4402*Math.pow(Temperatura_crudo_pseudocritica_Tc, 0.16666667)/(Math.pow(Peso_molecular_MW_crudo, 0.5)*Math.pow(Presion_crudo_pseudocritica_Pc,0.666667));
		...
		Ecuacion_11= Math.pow(X, 3)-(bm +Presion_crudo/Temperatura_crudo)*Math.pow(X, 2)+(am/Temperatura_crudo)*X-(am*bm/Temperatura_crudo);
		D_11= Math.pow(X_1, 3)-(bm +Presion_crudo/Temperatura_crudo)*Math.pow(X_1, 2)+(am/Temperatura_crudo)*X_1-(am*bm/Temperatura_crudo);
		D_21= Math.pow(X_2, 3)-(bm +Presion_crudo/Temperatura_crudo)*Math.pow(X_2, 2)+(am/Temperatura_crudo)*X_2-(am*bm/Temperatura_crudo);
		X_nuevo=(double)0;
